...
public class VmCloudletAssignermms extends VmCloudletAssigner{
	...
	public List<Cloudlet> cloudletAssign(List<Cloudlet> cloudletList,
		...
		if (vmList != null || vmList.size() != 0) {
			...
			for(Cloudlet c:toAssignCloudletList){
				...
				if(c.getCloudletLength()<1500){
					c.setSe(3);
				}else if(c.getCloudletLength()>1500&&c.getCloudletLength()<2500){
					c.setSe(2);
				}else{
					c.setSe(1);
				}
				
				if(c.getWaitingTime()<45){
					c.setWe(1);
				}else if(c.getWaitingTime()>45&&c.getWaitingTime()<60){
					c.setWe(2);
				}else{
					c.setWe(3);
				}
				...
				if(c.getDeadline()<50){
					c.setDe(3);
				}else if(c.getDeadline()>50&&c.getDeadline()<60){
					c.setDe(2);
				}else{
					c.setDe(1);
				}
			...
			for (i = 0; i < n; i++) {
				...
				//toAssignCloudletList.get(i).setCloudletLength(20000);
				for (int j = 0; j < m; j++) {
